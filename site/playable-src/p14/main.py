...
def build_music():
    # Browser port: the three tracks below were pre-rendered to OGG at build time
    # from the SAME _compose()/_arp_melody() synthesis used by the original, so the
    # audio is identical while avoiding tens of seconds of pure-Python sample
    # synthesis on the WASM interpreter (which would freeze the first frame).
    if not SOUND_ON:
        return
    for _name in ("compete", "chase", "celebrate"):
        try:
            MUSIC[_name] = pygame.mixer.Sound(f"music_{_name}.ogg")
        except Exception:
            pass
...
# ==========================================================================
async def run_title_card(title, level_no=None):
...
# ==========================================================================
async def main():
    set_selected_character(await run_start())     # start screen + character select
    await run_title_card("THE ALARM CLOCK", 1)          # Level 1
    await run_level1()
    await run_title_card("BRUSH OR GET BRUSHED", 2)      # Level 2
    await run_level2()
    await run_title_card("LUNCH GONE WRONG", 3)               # Level 3
    await run_level3()
    await run_title_card("STATIONERY MAYHEM", 4)             # Level 4
    await run_level4()
    await run_title_card("JOURNEY TO THE BEDROOM", 5)  # Level 5
    await run_level5()
    await run_end()                                      # completion screen
...
def speak(text):
    ...
    try:
        import platform
        platform.window.speechSynthesis.cancel()
        _u = platform.window.SpeechSynthesisUtterance.new(text)
        _u.rate = 1.05
        platform.window.speechSynthesis.speak(_u)
    except Exception:
        pass
...
def make_ring_sound():
    """Load the pre-rendered 'brrring' phone ring (bundled as ring.ogg).

    The original synthesised this WAV in-memory with the stdlib `wave` module,
    which pygbag's WASM CPython does not ship; the byte-identical waveform is
    pre-rendered to OGG at build time instead. Returns None on failure.
    """
    try:
        return pygame.mixer.Sound("ring.ogg")
    except Exception:
        return None
