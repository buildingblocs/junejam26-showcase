# Browser port of "Bleach Rush" (P8 - Most Silly).
# Gameplay is unchanged from the submitted source. The only edits versus the
# original are the ones required to run under pygbag in the browser:
#   * wrapped in `async def main()` with `await asyncio.sleep(0)` yields per loop
#   * hardcoded Windows asset paths re-pointed to bundled ./assets files
#   * window-close exits return cleanly instead of calling sys.exit()
import asyncio
import pygame
...
async def main():
    ...
    bg_surf = pygame.image.load("./assets/bg.png").convert()
    ...
    # Sprites are loaded once here instead of every frame so the browser build
    # keeps a steady frame rate; the rendered result is identical.
    player_model = pygame.image.load("./assets/player.png").convert_alpha()
    u_died = pygame.image.load("./assets/died.png").convert()
    badguy = pygame.image.load("./assets/enemy.png").convert_alpha()
    ...
    # ---------------------CUTSCENES-------------------------
    cutscene1 = pygame.image.load("./assets/cutscene1.png").convert()
    cutscene2 = pygame.image.load("./assets/cutscene2.png").convert()
    cutscene3 = pygame.image.load("./assets/cutscene3.png").convert()
    ...
    while running:
        ...
        if kill_counter == 10:
            pygame.mixer.music.stop()
            wasted_all_my_time_on_this = pygame.mixer.Sound("./assets/wasted.ogg")
